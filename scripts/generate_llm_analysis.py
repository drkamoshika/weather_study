...
def choose_models(api_key: str, requested: str | None) -> list[str]:
    ...
    # Stable Flash models are inexpensive and appropriate for a daily job. The list
    # comes from the API so newly introduced versions naturally move to the front.
    ordered=[name for name,_ in sorted(usable,key=lambda item:(item[1]=="STABLE","flash" in item[0],"preview" not in item[0],item[0]),reverse=True)]
    if requested and requested in ordered: ordered.remove(requested); ordered.insert(0,requested)
    elif requested: print(f"Gemini model {requested} is unavailable; trying an available model")
    return ordered
...
def generate(target_date: str, location_id: str, requested_model: str | None = None) -> bool:
    ...
    try:
        ...
        manifest["llm"]={"status":"success","model":model,"text_only_fallback":fallback,"attempts":attempts,"error":None}; write_json(manifest_file,manifest); print(f"Gemini analysis generated with {model} (text fallback={fallback})"); return True
    except Exception as error:
        (out/"llm-analysis.md").unlink(missing_ok=True)
        manifest["llm"]={"status":"failed","model":model,"text_only_fallback":fallback,"attempts":attempts,"error":{"type":type(error).__name__,"message":str(error)[:1000]}}; write_json(manifest_file,manifest); print(f"Gemini failed: {type(error).__name__}: {error}"); return False
