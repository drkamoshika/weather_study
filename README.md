...
「取得履歴」には日付、地点、自動取得／手動取得、保存資料数、Gemini解説の有無と使用モデルを表示します。履歴は縦スクロールでき、各行を選ぶとそのsnapshotを直接開けます。日付・地点を選ぶ別画面は設けず、取得履歴を過去snapshotの入口にしています。資料種別のチェック欄は、開いているsnapshot内の表示だけを切り替えます。
...
生成結果は `llm-analysis.md` と `llm-analysis.json` の両方へ保存します。JSONには実際のモデル、生成時刻、入力に使ったsource ID、Markdownパスを記録します。画面にも実際に使用したモデル名を表示します。解説は図を個別に紹介するのではなく、利用可能な全図・観測・予報を照合し、「観測された天気 → 地上気圧配置・前線 → 850/500hPa → 数値予報・航空資料」という因果関係で現在の天気を総合推論するよう指示しています。資料同士の一致・不一致、判断根拠、不確実性も明示させます。画像入力が使えない場合は同じモデルのtext-only、さらに失敗した場合は次のFlashへフォールバックします。
